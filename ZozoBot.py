...
class ZozoBot:
    ...
    # Get seckey
    def get_sec_key(self, account_num):
        file_seckey = f'ZozoRes/SecKey{account_num}.txt'
        file_seckey = str(self.PROJECT_ROOT / file_seckey)
        with open(file_seckey) as f:
            content = f.read().strip()
        return content
    ...
    def grab_cookie_header_sec(self, driver, account_num):
    ...
    def get_creds(self, account):
        account_num = str(account["AccountNo"])
        email = account["Email"]
        password = str(account["Password"])
        proxy = str(account["Proxy"])
        start_link = str(account["StartLink"])
        keyword = str(account["Keyword"])
        ...
        # counter = 0
        LOGGER.info(f"Waiting for credentials to be copied for account {email}")
        while True:
            done = input(f"Did you copied the credentials for account {email} y/n?")
            if done == 'y' or done == 'Y':
                break
            else:
                pass
    ...
    def add_to_cart(self, account):
        ...
        proxies = {"http": proxy, "https": proxy}
        # counter = 0
        cookies = {self.get_cookies(account_num=account_num)}
        headers = {self.get_headers(account_num=account_num)}
        seckey = {self.get_sec_key(account_num=account_num)}
        while True:
            # if counter == 3:
            #     break
            # counter += 1
...
def main():
    freeze_support()
    enable_cmd_colors()
    trial_date = datetime.datetime.strptime('2021-03-18 23:59:59', '%Y-%m-%d %H:%M:%S')
    # Print ASCII Art
    print('************************************************************************\n')
    ...
    if zozo_bot.trial(trial_date):
        ...
        if os.path.isfile(file_path_account):
            ...
            for account in account_list:
                zozo_bot.get_creds(account=account)
            # Start the process
